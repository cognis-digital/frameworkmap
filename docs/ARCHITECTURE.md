...
- **rules/analyzers** apply the heuristics shipped in `frameworkmap/core.py`.
- **score** ranks by severity.
- **MCP server** (`frameworkmap mcp`) exposes `scan` for Cognis.Studio agents.
